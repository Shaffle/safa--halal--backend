...
FULL_ENRICH_LIMIT = int(os.environ.get("FULL_ENRICH_LIMIT", "24"))
FULL_ENRICH_WORKERS = int(os.environ.get("FULL_ENRICH_WORKERS", "4"))
...
def wants_full_enrichment(payload):
    level = str(payload.get("enrichment_level", "")).lower()
    return (
        level in {"full", "deep", "complete"}
        or bool(payload.get("include_reviews"))
        or bool(payload.get("include_review_counts"))
    )
...
def best_rating(existing, incoming):
    existing_rating = float_or_none(existing)
    incoming_rating = float_or_none(incoming)
    if incoming_rating is None:
        return existing
    if existing_rating is None:
        return incoming_rating
    return incoming_rating if incoming_rating > 0 else existing_rating
...
def get_restaurants():
    try:
        ...
        full_enrichment = wants_full_enrichment(payload)
        key = cache_key("restaurants", {
        ...
        if full_enrichment:
            merged = enrich_restaurant_collection(merged, lat, lng, FULL_ENRICH_LIMIT)
...
def get_doordash_enrichment():
    try:
        ...
        if cached is not None:
            return jsonify(cached)
        response = {"enrichment": scrape_full_restaurant_enrichment(name, address, lat, lng)}
        cache_set(key, response)
        return jsonify(response)
    except Exception as e:
...
def scrape_full_restaurant_enrichment(name, address, lat, lng):
    enrichment = scrape_doordash_enrichment(name, address, lat, lng)
    public_reviews = scrape_public_review_signals(name, address, lat, lng)
    return merge_enrichment(enrichment, public_reviews)
...
def merge_enrichment(base, incoming):
    merged = dict(base or empty_doordash_enrichment())
    incoming = incoming or {}

    base_count = int_or_zero(merged.get("review_count"))
    incoming_count = int_or_zero(incoming.get("review_count"))
    merged["review_count"] = max(base_count, incoming_count)
    merged["reviewCount"] = merged["review_count"]
    merged["total_reviews"] = merged["review_count"]

    incoming_rating = incoming.get("rating")
    if incoming_rating is not None and incoming_count >= base_count:
        merged["rating"] = incoming_rating
    elif not merged.get("rating"):
        merged["rating"] = incoming_rating

    merged["google_review_count"] = int_or_zero(incoming.get("google_review_count"))
    merged["apple_review_count"] = int_or_zero(incoming.get("apple_review_count"))
    merged["doordash_review_count"] = int_or_zero(base.get("review_count") if base else 0)
    merged["sources"] = dedupe_strings((base or {}).get("sources", []) + incoming.get("sources", []))

    if not merged.get("categories") and incoming.get("categories"):
        merged["categories"] = incoming.get("categories")
    return merged


def scrape_public_review_signals(name, address, lat=None, lng=None):
    signals = []
    for source_name, url in public_review_urls(name, address, lat, lng):
        try:
            page = smart_get(url, prefer_stealth=True)
            text = page.body.text if page.body else ""
            script_text = "\n".join(normalize_embedded_json_text(script.text or "") for script in page.css("script"))
            rating, review_count = extract_review_signal(f"{text}\n{script_text}")
            ...
            print(f"{source_name} review signal skipped for '{name}': {e}")

    return combine_review_signals(signals)


def public_review_urls(name, address, lat=None, lng=None):
    from urllib.parse import quote_plus

    query = quote_plus(f"{name} {address}".strip())
    urls = [
        ("google", f"https://www.google.com/search?q={query}+reviews"),
        ("google", f"https://www.google.com/search?q={query}+restaurant+rating"),
        ("apple", f"https://maps.apple.com/?q={query}"),
    ]
    if lat and lng:
        urls.append(("apple", f"https://maps.apple.com/?q={query}&ll={lat},{lng}"))
    return urls


def combine_review_signals(signals):
    if not signals:
        return {
            "rating": None,
            "review_count": 0,
    ...
    best = max(signals, key=lambda signal: int_or_zero(signal.get("review_count")))
    google_count = max((int_or_zero(s.get("review_count")) for s in signals if s.get("source") == "google"), default=0)
    apple_count = max((int_or_zero(s.get("review_count")) for s in signals if s.get("source") == "apple"), default=0)
    return {
        "rating": best.get("rating"),
        "review_count": int_or_zero(best.get("review_count")),
        "reviewCount": int_or_zero(best.get("review_count")),
        "total_reviews": int_or_zero(best.get("review_count")),
        "google_review_count": google_count,
        "apple_review_count": apple_count,
        "sources": dedupe_strings([s.get("source") for s in signals if s.get("source")])
    }


def extract_review_signal(text):
    normalized = normalize_embedded_json_text(text or "")
    rating = extract_rating_from_text(normalized)
    review_count = extract_review_count_from_text(normalized)
    return rating, review_count


def extract_rating_from_text(text):
    patterns = [
        r'"(?:ratingValue|averageRating|starRating|rating)"\s*:\s*"?([0-5](?:\.\d+)?)"?',
        r'([0-5](?:\.\d+)?)\s*(?:stars?|rating)',
        r'Rated\s+([0-5](?:\.\d+)?)',
    ]
...
def extract_review_count_from_text(text):
    patterns = [
        r'"(?:reviewCount|ratingCount|ratingsCount|user_ratings_total|userRatingsTotal|aggregateRatingCount)"\s*:\s*"?([\d,]+)"?',
        r'([\d,]+)\s*(?:Google\s*)?(?:reviews?|ratings?)',
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, text, re.IGNORECASE):
            count = int_or_zero(match.group(1))
            if count > 0:
                counts.append(count)
...
def extract_doordash_rating(page):
    ...
    for script in page.css("script"):
        ...
        if rating is None:
            rating = extract_rating_from_text(script_text)
        if review_count == 0:
            review_count = extract_review_count_from_text(script_text)
    ...
    for dr in doordash:
        ...
        for zr in merged:
            ...
            if name_match and (distance_match or z_lat is None or d_lat is None):
                zr["rating"] = best_rating(zr.get("rating"), dr.get("rating"))
                zr["reviewCount"] = max(int_or_zero(zr.get("reviewCount")), int_or_zero(dr.get("reviewCount")))
                zr["review_count"] = zr["reviewCount"]
                zr["total_reviews"] = zr["reviewCount"]
...
def enrich_restaurant_collection(restaurants, lat, lng, limit):
    if not restaurants:
        return restaurants

    enriched = list(restaurants)
    target_indexes = list(range(min(len(enriched), max(0, limit))))
    if not target_indexes:
        return enriched

    with ThreadPoolExecutor(max_workers=max(1, FULL_ENRICH_WORKERS)) as executor:
        futures = {}
        for index in target_indexes:
            restaurant = enriched[index]
            name = sanitize_text(restaurant.get("name", ""))
            address = sanitize_text(restaurant.get("address", ""), max_length=240)
            r_lat = float_or_none(restaurant.get("latitude")) or lat
            r_lng = float_or_none(restaurant.get("longitude")) or lng
            if not name:
                continue
            futures[executor.submit(scrape_full_restaurant_enrichment, name, address, r_lat, r_lng)] = index
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"Full enrichment failed: {exc}")
                continue
            enriched[index] = merge_restaurant_enrichment(enriched[index], enrichment)

    return enriched


def merge_restaurant_enrichment(restaurant, enrichment):
    merged = dict(restaurant)
    if not enrichment:
        return merged

    existing_count = int_or_zero(merged.get("reviewCount") or merged.get("review_count"))
    enriched_count = int_or_zero(enrichment.get("review_count") or enrichment.get("reviewCount"))
    review_count = max(existing_count, enriched_count)
    merged["reviewCount"] = review_count
    merged["review_count"] = review_count
    merged["total_reviews"] = review_count

    if enriched_count >= existing_count and enrichment.get("rating"):
        merged["rating"] = enrichment.get("rating")
    elif not merged.get("rating") and enrichment.get("rating"):
        merged["rating"] = enrichment.get("rating")

    if enrichment.get("google_review_count"):
        merged["google_review_count"] = enrichment.get("google_review_count")
    if enrichment.get("apple_review_count"):
        merged["apple_review_count"] = enrichment.get("apple_review_count")
    if enrichment.get("doordash_review_count"):
        merged["doordash_review_count"] = enrichment.get("doordash_review_count")
    if enrichment.get("sources"):
        merged["review_sources"] = enrichment.get("sources")
    if not merged.get("galleryPhotos") and enrichment.get("photos"):
        merged["galleryPhotos"] = enrichment.get("photos")
    if not merged.get("coverImage") and enrichment.get("photos"):
        merged["coverImage"] = enrichment.get("photos")[0]
    if not merged.get("businessHours") and enrichment.get("business_hours"):
        merged["businessHours"] = enrichment.get("business_hours")
    return merged
